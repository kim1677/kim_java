...
<%
    ...
    Board data=null;	// select sql문의 결과는 Board클래스 객체 하나에 저장되는 데이터이기 때문에 board 클래스 객체 저장할 수 있는 변수만 있으면 된다.
    try{
    	db.pt = db.conn.prepareStatement(sql);
    	db.pt.setInt(1, id);
    	db.rs=db.pt.executeQuery();
    	if(db.rs.next() ){
    		data=new Board(db.rs.getInt("board_id"), db.rs.getString("writer"), db.rs.getString("title"), db.rs.getString("content"), 
								db.rs.getInt("hit"));
    	}
    ...
    try{
    	...
    	if(db.rs.next()){
    		img = new Boardimg(db.rs.getInt("board_img_id"),
    				db.rs.getInt("board_id"), db.rs.getString("img_name"), db.rs.getString("img_path"));
    	}
    }catch(SQLException e){
    	e.printStackTrace();
