...
<%-- 회원가입을 위해 입력값을 Member 클래스 객체로 저장하고 객체의 데이터를 데이터베이스에 저장한다. --%>
...
<%
	...
	try{
		...
		pt.setString(2, newMem.getUserPassword());	// 두번째 물음표 자리에 데이터넣기
		pt.setString(3, newMem.getUserEmail());
		pt.setString(4, newMem.getUserName());
		pt.executeUpdate();							// insert, update, delete 문을 사용시에는 excuteUpate();
		...
		out.println("<h1회원가입 오류! 다시입력!</h1>"+"<a href='javascript:history.back();'>이전</a>");
